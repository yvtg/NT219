...
def send_device_verification_email(email, fingerprint):
    ...
    token = ts.dumps(
        {"email": email, "fingerprint": fingerprint}, salt='device-verify')
    verify_link = f"http://localhost:8000/api/auth/verify-device?token={token}"
    msg = MIMEText(
        f"New device detected. Verify it here: {verify_link}\nLink expires in 10 minutes.")
    ...
    try:
        ...
        logging.error(
            f"Failed to send device verification email to {email}: {str(e)}")
        raise
...
def get_csrf_token():
    ...
    expired_tokens = [token for token,
                      expiry in csrf_tokens.items() if expiry < current_time]
...
def check_csrf_token():
    if request.method in ['POST', 'PATCH', 'PUT', 'DELETE']:
        ...
        if not csrf_token:
            logging.warning(
                f"No CSRF token in headers for request from {request.remote_addr}")
            return None

        if not cookie_csrf:
            logging.warning(
                f"No CSRF token in cookies for request from {request.remote_addr}")
            return None

        if csrf_token != cookie_csrf:
            logging.warning(
                f"CSRF token mismatch for request from {request.remote_addr}")
...
def verify_recaptcha(recaptcha_response):
    ...
    if not recaptcha_response:
        logging.warning(
            "verify_recaptcha được gọi nhưng không có recaptcha_response token.")
        return False
    ...
    logging.info(
        f"Gửi yêu cầu xác thực reCAPTCHA cho IP: {request.remote_addr}")
    ...
    try:
        ...
        if result.get('success'):
            ...
            logging.error(
                f"Xác thực reCAPTCHA thất bại với mã lỗi: {error_codes}")
            return False
...
def verify_email():
    ...
    c.execute(
        'SELECT verification_token FROM users WHERE email = %s AND verified = 0', (email,))
    user = c.fetchone()

    if user and user['verification_token'] == token:
        c.execute(
            'UPDATE users SET verified = 1, verification_token = NULL WHERE email = %s', (email,))
        conn.commit()
        conn.close()
...
def enable_2fa():
    ...
    try:
        ...
        c.execute(
            'UPDATE users SET totp_secret = %s, is_2fa_enabled = TRUE WHERE id = %s', (totp_secret, id))
        conn.commit()

        totp_uri = pyotp.totp.TOTP(totp_secret).provisioning_uri(
            name=id, issuer_name='Netflix')
        qr = qrcode.QRCode()
...
def verify_2fa():
    ...
    try:
        ...
        c = conn.cursor(cursor_factory=RealDictCursor)
        c.execute(
            'SELECT verified, totp_secret, is_2fa_enabled FROM users WHERE id = %s', (id,))
        user = c.fetchone()
        if not user:
...
def get_device_fingerprint(request):
    ...
    fingerprint = hashlib.sha256(
        f"{user_agent}:{accept_language}".encode()).hexdigest()
    return fingerprint
...
def disable_2fa():
    ...
    try:
        ...
        c.execute(
            'UPDATE users SET is_2fa_enabled = FALSE, totp_secret = NULL WHERE id = %s', (id,))
        conn.commit()
        conn.close()
...
def login():
    ...
    try:
        ...
        if existing_device:
            logging.info(
                f"Known device login for user {email} from fingerprint {fingerprint[:10]}...")
            ...
            logging.warning(
                f"New device detected for user {email}. Verification required.")
            ts = URLSafeTimedSerializer(os.getenv("SECRET_KEY"))
            verification_token = ts.dumps(
                {"user_id": user['id'], "fingerprint": fingerprint}, salt='device-verify-salt')
            try:
                ...
                msg = MIMEText(
                    f"We detected a login from a new device. If this was you, please verify the device by clicking this link: {verification_link}\nThis link is valid for 10 minutes.")
                ...
                with smtplib.SMTP(os.getenv('SMTP_SERVER'), int(os.getenv('SMTP_PORT'))) as server:
                    ...
                    server.login(os.getenv('SMTP_USERNAME'),
                                 os.getenv('SMTP_PASSWORD'))
                    server.sendmail(os.getenv('EMAIL_FROM'),
                                    email, msg.as_string())
            except Exception as e:
                logging.error(
                    f"Failed to send device verification email to {email}: {str(e)}")
...
def verify_device_token():
    ...
    try:
        ...
        data = ts.loads(token, salt='device-verify-salt',
                        max_age=600)  # 10 phút
        ...
        conn.commit()
        logging.info(
            f"Device with fingerprint {fingerprint[:10]}... verified for user_id {user_id}")
        ...
        logging.error(
            f"Invalid or expired device verification token: {str(e)}")
...
def magic_login():
    ...
    try:
        ...
        logging.info(
            f"User {email} logged in via magic link from device {fingerprint[:10]}...")
        return response
...
def check_session():
    ...
    c = conn.cursor(cursor_factory=RealDictCursor)
    c.execute(
        'SELECT last_ip, totp_secret, role FROM users WHERE id = %s', (user_id,))
    result = c.fetchone()
    conn.close()
    ...
    if last_ip and last_ip != current_ip:
        logging.warning(
            f"Suspicious session for {user_id}: IP changed from {last_ip} to {current_ip}")
...
def logout():
    ...
    try:
        ...
        logging.info(
            f"User {get_jwt_identity()} logged out, token {jti} blacklisted")
        response = make_response(
        ...
        response = make_response(
            jsonify({'message': 'Logout successful (token not blacklisted)'}), 200)
        response.delete_cookie('csrf_token')
        return response
    finally:
        if conn:
...
def get_content():
    ...
    try:
        if role == 'user':
            c.execute(
                'SELECT id, title, video_url, thumbnail_url FROM videos LIMIT 6')
            content = c.fetchall()
        elif role == 'admin':
            c.execute(
                'SELECT email, username, role, verified FROM users LIMIT 10')
            content = c.fetchall()
        elif role == 'production':
            c.execute(
                'SELECT id, title, video_url, thumbnail_url, category FROM videos LIMIT 10')
            content = c.fetchall()
        else:
            return jsonify({'message': 'Invalid role'}), 403

        response = make_response(
            jsonify({'role': role, 'content': content}), 200)
        return response
    except Exception as e:
...
def logout_device(device_id):
    ...
    try:
        ...
        logging.info(
            f"Logging out device. Is it the current device? {is_current_device}")
        ...
        logging.info(
            f"User {user_id_from_token} logged out device {device_id}")
        ...
        logging.error(
            f"Error logging out device {device_id} for user {user_id_from_token}: {e}")
...
def find_or_create_oauth_user(email, name, provider, oauth_id):
    ...
    c.execute("SELECT * FROM users WHERE oauth_provider = %s AND oauth_id = %s",
              (provider, oauth_id))
    user = c.fetchone()
    if user:
        conn.close()
...
def callback_google():
    ...
    client_config = {
        "web": {
            ...
            "client_secret": os.getenv("GOOGLE_CLIENT_SECRET"),
            "token_uri": "https://oauth2.googleapis.com/token",
            # Thêm lại để đảm bảo tính đầy đủ
            "auth_uri": "https://accounts.google.com/o/oauth2/auth",
        }
    }
